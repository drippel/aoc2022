...
object Day03a {
  ...
  var scoreTable = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"

  def main(args: Array[String]): Unit = {
    Console.out.println("day03a...")

    var lines = data.split('\n').toList.map(_.trim)

    Console.out.println(lines)
    val items = lines.map(findItem(_))
    Console.out.println(items)
    val scores = items.map(score(_))
    Console.out.println(scores)
    Console.out.println(scores.sum)
  }

  def findItem( line : String ) : Char = {

    val l = line.length

    val (a,b) = line.splitAt(l/2)
    Console.out.println( s"{$a} {$b}" )

    val as = a.toSet
    val bs = b.toSet

    as.intersect(bs).head
  }

  def score( c : Char ) : Int = {
    var p = scoreTable.indexOf(c)
    p + 1
  }
  ...
  var data = """WVHGHwddqSsNjsjwqVvdwZRCbcJcZTCcsZbLcJJsCZ
