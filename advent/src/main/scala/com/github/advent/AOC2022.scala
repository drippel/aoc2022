...
object AOC2022 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var ls = data.split('\n').toList
    var elves = parse( ls, List(), List() )

    Console.println( elves )

    val sums = for( e <- elves ) yield {
      var is = e.map( i => i.toInt )
      is.sum
    }
    ...
    var ss = sums.sorted.reverse

    var winner = ss.head

    Console.println( winner )

    var (top3,theRest) = ss.splitAt(3)
    Console.println( top3 )
    Console.println( top3.sum )
