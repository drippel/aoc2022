...
object Day03b {
  ...
  def main(args: Array[String]): Unit = {
    ...
    groups.foreach(Console.out.println(_) )

    var bs = groups.map( findBadge( _ ) )
    Console.out.println(bs)

    var scores = bs.map(score(_))

    Console.out.println(scores.sum)

  }

  def findBadge( lines : List[String] ) : Char = {
    val a = lines(0).toSet
    val b = lines(1).toSet
    val c = lines(2).toSet

    val i = a.intersect(b)
    val badge = i.intersect(c)
    badge.head
  }
