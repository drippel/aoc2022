...
object Day07a {
  ...
  def calcSizes( dir : Directory ) : BigDecimal = {
    ...
    val ds = dir.dirs.map( calcSizes( _ ) ).sum
  ...
  def printDir( dir : Directory, level : String ) : Unit = {
    Console.out.println( s"${level} - ${dir.name} - size: ${dir.sizes.head} (dir)")
    for( d <- dir.dirs ) {
      printDir( d, level + "  ")
    }
    for( f <- dir.files ) {
      Console.out.println( s"${level}  - ${f.name} (file, ${f.size})")
    }
  }
  ...
  def parse( olines : List[String] ) : Directory = {
    ...
    val dnames = mutable.HashSet[String]()
    val fnames = mutable.HashSet[String]()
    ...
    def innerParse( lines : List[String], dirs : mutable.Stack[Directory] ) : Unit = {
      ...
      if (lines.isEmpty) {
        ...
        if (line.startsWith("$ cd ..")) {
          ...
          val cd = dirs.top
          if( "dir".equals(ps(0))) {
            Console.out.println("add dir")
            if( dnames.contains(ps(1)) ) {
              Console.out.println("dupe dir:" + dnames.contains(ps(1)))
            }
            dnames += ps(1)
            ...
            if( fnames.contains(ps(1)) ) {
              Console.out.println("dupe file:" + fnames.contains(ps(1)))
            }
    ...
    innerParse(olines.tail, mutable.Stack(root))
